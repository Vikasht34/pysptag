"""Test SPANN with replication (exact C++ SPTAG)"""
import numpy as np
import time
import sys
...
print("="*80)
print("SPANN + RaBitQ + REPLICATION (Exact C++ SPTAG)")
print("="*80)
...
# Build index with replication
print("\nBuilding SPANN with 8× replication + quantization...")
t0 = time.time()
index = SPANNRaBitQReplica(dim=dim, target_posting_size=1000, replica_count=8, bq=4)
...
print(f"  Posting sizes: min={min(len(p) for p in index.posting_lists)}, "
      ...
      f"avg={np.mean([len(p) for p in index.posting_lists]):.1f}")

# Search with C++ parameters
print(f"\nSearching {n_test} queries (search_internal_result_num=64, maxCheck=4096)...")
t0 = time.time()
recalls = []
...
print("\n" + "="*80)
print("✓ EXACT C++ SPTAG IMPLEMENTATION")
print("="*80)
